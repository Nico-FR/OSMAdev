...
def export_bedgraph(chrom, values, filepath, compressed):
    """Exports 1D track values as a single-column file containing only the value column."""
    df_bg = pd.DataFrame({
        'value': values
    })
    if compressed:
        df_bg.to_csv(filepath + '.gz', sep='\t', header=False, index=False, compression='gzip')
    else:
        df_bg.to_csv(filepath, sep='\t', header=False, index=False)
